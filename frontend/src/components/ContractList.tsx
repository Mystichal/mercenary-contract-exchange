...
export default function ContractList({ onCreateClick }: Props) {
  ...
  // Read the shared Registry object
  const { data: registry, isLoading } = useSuiClientQuery("getObject", {
    id: REGISTRY_ID,
    options: { showContent: true },
  });

  // Query ContractCreatedEvents to find all contracts
          ...
          return (
            <div key={ev.id.txDigest} style={{
              ...
              {account && p?.issuer !== account.address && (
                <button
                  onClick={() => acceptContract(ev.id.txDigest, "0x2::sui::SUI")}
                  style={{
